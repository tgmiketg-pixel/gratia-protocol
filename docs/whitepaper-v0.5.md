...
Additionally, the node must have a valid Proof of Life for the current day and must have staked the minimum required GRAT.
...
Stops small-scale multi-device gaming. Even if someone manages to fake Proof of Life on a few devices (by hiring people to carry phones, for example), they must also commit economic stake to each device. The capped staking model (see Section 6.3) ensures that splitting capital across multiple devices yields diminishing returns compared to honest single-device participation.
...
**Minimum stake:** A minimum amount of GRAT must be staked to participate in mining. This serves as an anti-spam mechanism and ensures every mining node has something to lose if they act dishonestly. The minimum is set low enough to be accessible to any participant and may be adjusted by governance as the token's value changes.

**Per-node stake cap:** Each node has a maximum effective stake — for example, 1,000 GRAT. Staking up to this cap contributes directly to the node's economic commitment and is subject to slashing if the node acts dishonestly.
...
- **ARM-optimized bytecode:** GratiaVM compiles to a custom bytecode set optimized for ARM NEON instructions, achieving near-native performance on mobile chipsets.
...
- **Mobile-native capabilities:** Unlike the EVM, GratiaVM has native opcodes for location queries, NFC triggers, proximity verification, and time-of-day logic — capabilities that are impossible on server-based blockchains.
...
- **Language:** GratiaScript — a TypeScript-derived language with mobile-native primitives (`@location`, `@proximity`, `@presence`, `@sensor`) that compile to GratiaVM bytecode. Familiar syntax lowers the barrier for the millions of existing TypeScript/JavaScript developers.
...
- **Layer 0 — Mesh Layer:** Bluetooth and Wi-Fi Direct mesh for offline resilience and local peer discovery.
...
1. All nodes currently in Mining Mode with valid Proof of Life and staked minimum are eligible.
...
- **Zero-knowledge attestations:** The protocol uses zero-knowledge proofs to attest that a node has valid Proof of Life, is in a valid location, and is running on real hardware — without revealing any underlying data.
...
Gratia runs on both Android and iOS. The application architecture ensures that both platforms are fully supported with approximately 80% shared code.
...
On Android, this layer is written in Kotlin using Android Sensor APIs and Jetpack Compose for the interface. On iOS, it is written in Swift using Core Motion, Core Location, Core Bluetooth, and SwiftUI for the interface.
...
The protocol is backed by 634 automated tests across the full workspace:
...
- ✅ 634 passing tests with zero warnings across the full workspace
...
- ✅ Mining Mode activation (plugged in + battery ≥ 80% + valid PoL + minimum stake)
...
- ✅ GratiaVM smart contract engine: MockRuntime with 3 demo contracts demonstrating @location, @proximity, @presence opcodes; ARM-calibrated gas metering; sandboxing with memory/time/call-depth limits
- ✅ Block explorer web app (HTML/CSS/JS) with live HTTP API serving real chain data from phones
- ✅ Network simulation tests: 51 tests covering phone farm detection, Sybil resistance, behavioral spoofing, emulator TEE penalties, stake manipulation edge cases

### Phase 3 — Public Testnet (Next)
- Bootstrap server for internet-wide peer discovery (beyond same-LAN mDNS)
- Full WASM execution via wasmer (currently MockRuntime with native handlers)
- GratiaScript compiler: TypeScript-derived language → WASM bytecode
- RocksDB persistent state storage (replacing file-based InMemoryStore serialization)
- iOS app via Swift sensor layer + same Rust core (UniFFI Swift bindings)
- Dynamic transaction fee system based on congestion and computation
- Security audit of Rust core, ZK proofs, and consensus mechanism

### Phase 4 — Mainnet (Months 13-18)
- Mainnet launch with genesis block mined on real phones under same rules as everyone
- Android and iOS apps in respective app stores (positioned as "wallet with rewards")
- Geographic sharding activation (4-20 shards, up to 2,560 TPS)
- Bluetooth/Wi-Fi Direct mesh layer for offline transactions
- Oracle layer for aggregated environmental sensor data
- Bug bounty program

### Phase 5 — Scale (Year 2+)
- Target: 1 million active mining nodes
- Cross-chain bridges to Bitcoin and Ethereum ecosystems
- Developer ecosystem grants and GratiaScript SDK
...
*This document describes a working protocol (v0.5) that has been implemented and tested on real hardware. All parameters, thresholds, and mechanisms described herein have been validated through 634 automated tests and real-device deployment. Specifications remain subject to revision based on ongoing research, security analysis, and community input.*
